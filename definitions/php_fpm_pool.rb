...
define :php_fpm_pool, :template => "pool.conf.erb", :enable => true do
  ...
  include_recipe 'cog_php-fpm'

  conf_file = "#{node['cog_php-fpm']['pool_conf_dir']}/#{pool_name}.conf"

  if params[:enable]
    template conf_file do
      only_if "test -d #{node['cog_php-fpm']['pool_conf_dir']} || mkdir -p #{node['cog_php-fpm']['pool_conf_dir']}"
